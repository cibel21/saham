...
FIX: Replaced all hex+alpha (e.g. "#FF335588") with proper rgba() strings,
     which are required by Plotly 5.x / Python 3.14 environment.
"""
...
# ── COLOR PALETTE (solid hex only — no alpha appended) ────────────────────────
COLORS = {
    "bg":         "#0E1117",
    "bg_sec":     "#1E1E2E",
    "grid":       "#2A2A3E",
...
# Pre-defined rgba variants used throughout
RGBA = {
    "bull_fill":      "rgba(0,255,136,0.12)",
    "bear_fill":      "rgba(255,51,85,0.12)",
    ...
    "bear_step1":     "rgba(255,51,85,0.20)",
    "accent3_step":   "rgba(255,107,157,0.20)",
    "neutral_step":   "rgba(255,221,0,0.20)",
    "bull_step1":     "rgba(0,255,136,0.20)",
    "bull_step2":     "rgba(0,255,136,0.40)",
    "hover_bg":       "rgba(26,26,46,1)",
    "legend_bg":      "rgba(26,26,46,1)",
...
# Separate legend config — only applied to charts that have traces (not Indicator)
LEGEND_DEFAULTS = dict(
    legend=dict(
        bgcolor=RGBA["legend_bg"],
...
def create_candlestick_chart(df: pd.DataFrame, signals: dict, show_ma: bool = True) -> go.Figure:
    ...
    fig.add_trace(go.Candlestick(
        ...
        increasing_fillcolor="rgba(0,255,136,0.55)",
        decreasing_fillcolor="rgba(255,51,85,0.45)",
        line=dict(width=1),
    ), row=1, col=1)
    ...
    if "BB_Upper" in df.columns and df["BB_Upper"].notna().any():
        fig.add_trace(go.Scatter(
            ...
            name="BB Upper",
            line=dict(color=RGBA["bb_line"], width=1, dash="dot"),
            showlegend=False,
        ), row=1, col=1)
    ...
    # ── VOLUME BARS ───────────────────────────────────────────────────────────
    colors_vol = [
        COLORS["volume_pos"] if float(c) >= float(o) else COLORS["volume_neg"]
    ...
    fig.update_layout(
        ...
        **LEGEND_DEFAULTS,
        ...
        hovermode="x unified",
        hoverlabel=dict(bgcolor=RGBA["hover_bg"], font_color=COLORS["text"]),
    )
...
def create_rsi_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_hrect(y0=70, y1=100, fillcolor=RGBA["overbought"],
                  line_width=0, annotation_text="Overbought",
                  annotation_position="right")
    fig.add_hrect(y0=0,  y1=30,  fillcolor=RGBA["oversold"],
                  line_width=0, annotation_text="Oversold",
                  annotation_position="right")
    fig.add_hrect(y0=30, y1=70,  fillcolor=RGBA["neutral_fill"], line_width=0)
    ...
    fig.update_layout(
        **LAYOUT_DEFAULTS,
        ...
        showlegend=False,
        legend=dict(bgcolor=RGBA["legend_bg"], bordercolor=COLORS["grid"],
...
def create_macd_chart(df: pd.DataFrame) -> go.Figure:
    ...
    hist_colors = [
        COLORS["bull"] if float(v) >= 0 else COLORS["bear"]
...
def create_score_gauge(score: float) -> go.Figure:
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "bordercolor": COLORS["grid"],
            "steps": [
                {"range": [0,  25], "color": RGBA["bear_step1"]},
                {"range": [25, 45], "color": RGBA["accent3_step"]},
                {"range": [45, 65], "color": RGBA["neutral_step"]},
                {"range": [65, 80], "color": RGBA["bull_step1"]},
                {"range": [80,100], "color": RGBA["bull_step2"]},
            ],
            "threshold": {
    ...
    # Indicator figures must NOT use plot_bgcolor — use paper_bgcolor only
    fig.update_layout(
        paper_bgcolor=COLORS["bg"],
        font=dict(family="IBM Plex Mono, monospace", color=COLORS["text"], size=11),
        height=230,
...
def create_sub_scores_bar(score_dict: dict) -> go.Figure:
    ...
    bar_colors = [
        COLORS["bull"]    if v >= 65 else
        (COLORS["neutral"] if v >= 45 else COLORS["bear"])
        for v in values
    ]
